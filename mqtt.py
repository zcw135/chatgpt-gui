...
def getcbot():
    return Chatbot(email="jecom46461@fom8.com", password="techi@721",insecure=False)
...
with tab2:
    ...
    def greet(name,formula,mode):
        ...
        if mode == "Organise":
            ...
            async def main():
                bot = Chatbot(email="jecom46461@fom8.com", password="techi@721",insecure=False)

                oddy = ""
                async for response in  bot.ask(os.environ['CUSTOM_BASE_PROMPT'] + "\n" +  name):
                    oddy = oddy + response["choices"][0]["text"].replace("<|im_end|>", "")
                    ress.write(oddy)
                return oddy     
            asyncio.run(main()) 
        if mode == "Rephrase":
        ...
        if mode == "Chat":
            ...
            async def main():
                ...
                oddy = ""
                async for response in  bot.ask(name):
                    oddy = oddy + response["choices"][0]["text"].replace("<|im_end|>", "")
                    ress.write(oddy)
                return oddy     
            asyncio.run(main()) 
    ...
    def chat(name,formula,mode, pres,freq,resp,temp):
        ...
        client.publish(sub_topic, str(name),qos=2)

        msg = subscribe.simple("vimo/send", hostname='broker.hivemq.com')
